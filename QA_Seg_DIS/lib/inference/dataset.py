...
def patch_generator(image, patch_indices, patch_size, batch_size, device):
    ...
    for i in range(steps):
        ...
        cmap_batch = extract_patches(cmap, patch_idxs, patch_size, "symmetric")
        ...
        cmap_batch = torch.from_numpy(cmap_batch).to(device=device, dtype=torch.float)
        yield image_batch, cmap_batch
